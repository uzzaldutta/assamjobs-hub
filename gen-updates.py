        ...
        <div className="flex-1 space-y-12">
           ...
           <section id="results">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                 {results?.map(res => <ResultCard key={res.id} data={res} compact={false} />)}
                 {(!results || results.length === 0) && <div className="col-span-full py-8 text-slate-500 italic">No recent results found.</div>}
              </div>
           </section>
           ...
           <section id="admit-cards">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                 {admitCards?.map(adm => <AdmitCard key={adm.id} data={adm} compact={false} />)}
                 {(!admitCards || admitCards.length === 0) && <div className="col-span-full py-8 text-slate-500 italic">No recent admit cards found.</div>}
              </div>
           </section>
           ...
           <section id="admissions">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                 {admissions?.map(adm => <AdmissionCard key={adm.id} data={adm} compact={false} />)}
                 {(!admissions || admissions.length === 0) && <div className="col-span-full py-8 text-slate-500 italic">No recent admissions found.</div>}
              </div>
           </section>
           ...
           <section id="scholarships">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                 {scholarships?.map(sch => <ScholarshipCard key={sch.id} data={sch} compact={false} />)}
                 {(!scholarships || scholarships.length === 0) && <div className="col-span-full py-8 text-slate-500 italic">No recent scholarships found.</div>}
              </div>
           </section>
           ...
           <section id="tenders">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                 ...
                 {(!tenders || tenders.length === 0) && <div className="col-span-full py-8 text-slate-500 italic">No recent tenders found.</div>}
              </div>
           </section>
...
"""
with open("src/app/updates/page.tsx", "w", encoding="utf-8") as f:
    f.write(code)
