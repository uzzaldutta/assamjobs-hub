...
export default async function FeedEntryDetail({ params }: { params: { id: string } }) {
  ...
  const approveAction = approveQueueItemAction.bind(null, item.id, item.payload, item.duplicate_of ? 'UPDATE' : 'NEW');
...
"""
import os
os.makedirs("src/app/admin/studio/ingestion/queue/[id]", exist_ok=True)
