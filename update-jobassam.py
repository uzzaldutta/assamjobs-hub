...
export class JobAssamAdapter implements SourceAdapter {
  ...
  async extract(raw: RawContent): Promise<any> {
    ...
    $('.entry-content a').each((i, el) => {
      ...
      if (linkText.includes('apply') || linkText.includes('online application') || linkText.includes('download')) {
        applyUrl = href;
...
"""
with open("src/lib/ingestion/adapters/JobAssamAdapter.ts", "w", encoding="utf-8") as f:
    f.write(code)
