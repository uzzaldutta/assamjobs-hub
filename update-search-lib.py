...
with open("src/lib/search/searchTypes.ts", "r", encoding="utf-8") as f:
    content = f.read()
...
# Update globalSearch.ts
import re
with open("src/lib/search/globalSearch.ts", "r", encoding="utf-8") as f:
    api = f.read()

api = api.replace('import { SearchResultItem } from "./searchTypes";', 'import { SearchResultItem, PaginatedSearchResult } from "./searchTypes";')
...
# Replace the entire executeGlobalSearch function
api = re.sub(r'export async function executeGlobalSearch.*?\}', new_api, api, flags=re.DOTALL)
